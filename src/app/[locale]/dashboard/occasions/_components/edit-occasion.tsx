...
export default function EditOccasion({ occasionId }: { occasionId: string }) {
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    ...
    if (data.image && data.image.length > 0) {
      formData.append("image", data.image[0]); // أول ملف
    }

    const res = await fetch(`${process.env.NEXT_PUBLIC_API}/occasions/${occasionId}`, {
      method: "PUT",
      body: formData,
      headers: {
        ...(await getAuthHeader()),
  ...
  return (
    <div className="flex flex-col gap-6 mt-24 bg-zinc-50">
      <header>
        <h4 className="font-semibold text-2xl text-zinc-800">Update Occasion: Wedding</h4>
      </header>
