...
const SearchInput = React.forwardRef<HTMLInputElement, React.ComponentProps<"input">>(
    ({ className, ...props }, ref) => {
        return (
            <label htmlFor="SearchInput" className="relative flex items-center justify-center ">
                {/* SearchInput */}
                <input
                    type={'search'}
                    className={cn(
                    ...
                    placeholder="Search..."
                />
                <span className="text-sm text-zinc-800 absolute start-0 p-2 z-10 flex flex-wrap gap-2 justify-center items-center"> <Search className="text-zinc-400" /> </span>
            </label>
        )
    }
)
